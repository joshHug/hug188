...
We can now take another step outwards and consider the full equation for q-value. Knowing $[R(s, a, s') + \gamma V^*(s')]$ represents the utility attained by acting optimally after arriving in state $s'$ after taking action $a$ from $s$, it becomes evident that the quantity
...
\begin{eqnarray*}
	\pi_{1}(cool) &=& {\operatorname{argmax}}\{slow: 1 \cdot [1 + 0.5 \cdot 2],\:\: fast: 0.5 \cdot [2 + 0.5 \cdot 2] + 0.5 \cdot [2 + 0.5 \cdot 2]\} \\
				  ...
				  &=& \boxed{fast} \\
	\pi_{1}(fast) &=& {\operatorname{argmax}}\{slow: 0.5 \cdot [1 + 0.5 \cdot 2] + 0.5 \cdot [1 + 0.5 \cdot 2] ,\:\: fast: 1 \cdot [-10 + 0.5 \cdot 0]\} \\
